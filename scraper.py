...
entity_id = "FTRR1X_HENHSFT_gov"
url = "http://www.heartofengland.nhs.uk/large-trust-payments/"
errors = 0
data = []
...
for block in arch_blocks:
    if '.csv' in block['href'] or '.xls' in block['href'] or '.xlsx' in block['href'] or '.pdf' in block['href']:
        ...
        csvMth = title[-2][:3]
        csvYr = title[-1][-4:]
        arch_link = block['href']
        html = urllib2.urlopen(arch_link)
        soup = BeautifulSoup(html, 'lxml')
        link = soup.find('p', 'attachment').find('a')['href']
        csvMth = convert_mth_strings(csvMth.upper())
        data.append([csvYr, csvMth, link])
